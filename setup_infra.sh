...
docker exec -d user_behavior-airflow-webserver-1 airflow connections add 'postgres_default' --conn-type 'Postgres' --conn-login 'airflow' --conn-password 'airflow' --conn-host 'localhost' --conn-port 5432 --conn-schema 'airflow'

echo "adding S3 bucket name to Airflow variables"
docker exec -d user_behavior-airflow-webserver-1 airflow variables set BUCKET $1
...
docker exec -d user_behavior-airflow-webserver-1 airflow variables set EMR_ID $EMR_CLUSTER_ID

echo "Setting up AWS access for Airflow workers"
AWS_ID=$(aws configure get aws_access_key_id)
AWS_SECRET_KEY=$(aws configure get aws_secret_access_key)
AWS_REGION=$(aws configure get region)
docker exec -d user_behavior-airflow-webserver-1 airflow connections add 'aws_default' --conn-type 'aws' --conn-login $AWS_ID --conn-password $AWS_SECRET_KEY --conn-extra '{"region_name":"'$AWS_REGION'"}'
